"""
Example smtpd events handler for the Spiral application.

Serves the mx-spiral service from config/smtpd.yaml: accepts mail
...
class SpiralSmtpdEvents(TokeoSmtpdEvents):
    ...
    def on_mail_from_event(self, ctx, mail_from_data):
        self.app.log.info(f'mx-spiral: mail from {mail_from_data}')
        # cleanup the address
    ...
    def on_rcpt_to_event(self, ctx, rcpt_to_data):
        self.app.log.info(f'mx-spiral: rcpt to {rcpt_to_data}')
        # cleanup the address
    ...
    def on_message_data_event(self, ctx):
        ...
        if match:
            url = match.group(0)
            self.app.log.info(f'mx-spiral: enqueue tasks count_words for {url}')
