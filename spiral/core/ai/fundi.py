"""
Fundi ai provider for the Spiral application.
...
lab lives in ``spiral/core/fundi``), running in-process with plain NumPy -- no
...
``spiral/core/fundi/weights.npz``; without them this provider raises a clear
...
class TokeoAiFundiProvider(TokeoAiProvider):
    ...
    def _plan(self, prompt, tools):
        ...
        try:
            from spiral.core.fundi.dsl import parse
            from spiral.core.fundi.infer import FundiModel
        ...
        if self._engine is None:
            try:
                ...
                raise TokeoAiError(
                    "fundi has no trained weights yet -- run 'python -m spiral.core.fundi.train' to create spiral/core/fundi/weights.npz"
                )
