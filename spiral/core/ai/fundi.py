...
lab lives in ``spiral/app/fundi``), running in-process with plain NumPy -- no
...
``spiral/app/fundi/weights.npz``; without them this provider raises a clear
...
class TokeoAiFundiProvider(TokeoAiProvider):
    ...
    def _plan(self, prompt, tools):
        ...
        try:
            from spiral.app.fundi.dsl import parse
            from spiral.app.fundi.infer import FundiModel
        ...
        if self._engine is None:
            try:
                ...
                raise TokeoAiError("fundi has no trained weights yet -- run 'make fundi-train' to create spiral/app/fundi/weights.npz")
