"""
Akili ai provider for the Spiral application.
...
lab lives in ```spiral/core/akili```), running in-process with plain NumPy -- no
...
```python -m spiral.core.akili.train```. This will create or update
```spiral/core/akili/weights.npz```; without them this provider raises a clear
...
class TokeoAiAkiliProvider(TokeoAiProvider):
    ...
    def _plan(self, prompt, tools):
        ...
        try:
            from spiral.core.akili.dsl import parse
        ...
        if self._engine is None:
            try:
                ...
                raise TokeoAiError(
                    "akili has no trained weights yet -- run 'python -m spiral.core.akili.train' to create spiral/core/akili/weights.npz"
                )
