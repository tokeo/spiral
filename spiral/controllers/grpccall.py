"""
gRPC client controller for the Spiral service.
...
from ..core.grpc.proto import spiral_pb2
from ..core.grpc.proto import spiral_pb2_grpc
...
class GrpcCallController(Controller):
    ...
    def _grpc_channel(self):
        ...
        - **SpiralError**: When only one of --cert/--key is set
        ...
        if bool(pargs.cert) != bool(pargs.key):
            raise SpiralError('--cert and --key must both be given for mTLS')
    ...
    def _grpc_metadata(self):
        ...
        - **SpiralError**: When --password is given without --user

        """
        pargs = self.app.pargs
        if pargs.user is None:
            if pargs.password is not None:
                raise SpiralError('--password requires --user')
            return None
    ...
    def count_words(self):
        ...
        with self._grpc_channel() as channel:
            stub = spiral_pb2_grpc.SpiralStub(channel)
            _ = stub.CountWords(
                spiral_pb2.CountWordsRequest(url=self.app.pargs.url),
                metadata=self._grpc_metadata(),
            )
