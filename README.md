...
language model (~600K parameters, ~2.5 MB) that plans calendar tool calls through the same guarded
...
- ```spiral/core/ai/``` - AI providers and plain-function tools behind the guarded contracts
...
- ```spiral/pact/ai/``` - AI util-functions necessary for sandboxes
...
- **Akili** is generated by ```tokeo generate``` from Ramani's template and placed into your application for demonstration purposes under ```spiral/core/akili/```.

<br/>

## ⚖️ Licensing

Spiral is demo material, published under the Apache License, Version 2.0. Take what you need: Apache-2.0 lets you use, modify and redistribute the code, including commercially, as long as you keep the notices it asks for. Any project you generate yourself with ```tokeo generate``` carries no license of its own, so there you are free to choose.
...
The module ```spiral.core.akili``` is meant as a demonstration and is not intended for delivery. It ships so you can experiment and learn, and it can be removed without side effects.

If you really have reasons to keep ```akili``` in your own project, its files must remain under the Apache License, Version 2.0. Make sure to comply with that license. Keep the file headers and the ```LICENSE.txt``` copy in the ```spiral/core/akili``` directory.
