...
def test_spiral_ai_config_lints_clean():
...
def test_spiral_ai_tools_exec(scratch):
    ...
    with SpiralAiTestApp() as app:
        ...
        # the date tools take a date-only string as utc, so the calendar step is
        # stable on every machine; clamping shows jan 31 + 1m -> feb 28,
        # jun 8 - 4m -> feb 8, feb 29 + 1y -> feb 28
...
def test_spiral_ai_trace_records_the_whole_run(scratch):
...
def test_spiral_ai_tool_schema_validator_denies_when_strict():
...
def test_spiral_ai_tool_schema_validator_flags_when_not_strict():
...
def test_spiral_ai_redact_guard_masks_secrets():
...
def test_spiral_ai_redact_guard_raises_on_missing_pattern():
...
def test_spiral_ai_truncate_transformer_caps_long_results():
    ...
    with SpiralAiTestApp() as app:
        transformer = SpiralAiTruncateTransformer(app)
