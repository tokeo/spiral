...
def test_spiral_akili_lexicon_loads():
    ...
    from spiral.core.akili.data import _LEXICON
    from spiral.core.akili.dsl import DOMAIN
    ...
    from spiral.core.akili.data import dataset
    from spiral.core.akili.infer import PLAN_BUDGET

    longest = max(len(d) for _, d in dataset(8000, seed=7) if d != '<nomatch>')
    assert longest + 1 <= PLAN_BUDGET, (longest, PLAN_BUDGET)


def test_spiral_akili_data_contract():
    ...
    from spiral.core.akili.data import dataset, _LEXICON
    from spiral.core.akili.dsl import DOMAIN, NOMATCH, Constrainer, parse
...
def test_spiral_akili_domain_covers_calendar_tools():
...
def test_spiral_akili_current_reports_utc_not_local_at_midnight(monkeypatch):
    ...
    from spiral.core.ai.tools.current import TokeoAiCurrentTool

    frozen = datetime(2026, 7, 3, 23, 30, tzinfo=timezone.utc)
    monkeypatch.setattr('spiral.core.ai.tools.current.utc_now', lambda: frozen)
...
@pytest.mark.skipif(
    not Path('spiral/core/akili/weights.npz').exists(),
    reason="akili has no trained weights yet (run 'python -m spiral.core.akili.train')",
)
def test_spiral_ai_akili_model():
    # the project's own trained micro language model (spiral/core/akili)
